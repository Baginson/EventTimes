...
3. **Secrets** — grep the diff for anything that looks like a hardcoded API key, password, or token. Confirm `VITE_FIREBASE_*` and `VITE_TICKETMASTER_API_KEY` are only ever read via `import.meta.env`, never string-literal fallback values. Confirm no `.env`/`.env.local` file is staged in git (`git status`/`git diff --stat` should never show it — `.gitignore` should still list `.env`, `.env.local`, `.env.*.local`).
