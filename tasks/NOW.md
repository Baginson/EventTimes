...
**Runda 3 (2026-07-21, po pozytywnym teście loginu)**: użytkownik potwierdził, że login nazwą użytkownika działa (odhaczone w `docs/QA_CHECKLIST.md`). Pierwsza próba doprecyzowania gestu #7 (wykrywanie prędkości "gdziekolwiek" w treści, próg 1300px/s) **okazała się zbyt czuła na realnym telefonie** — zwykłe przewijanie prawie zawsze przekraczało próg, scrollowanie stron stało się bardzo utrudnione.

**Runda 4 (2026-07-21, po negatywnym teście)**: na wyraźną prośbę użytkownika ("przeanalizuj, nie strzelaj z rozwiązaniem, weź inspirację z Facebooka/Google Maps") przeanalizowano realne zachowanie tych aplikacji — obie używają wzorca **uchwyt (zawsze przeciągalny) + pull-from-top (dopiero od scrollTop=0)**, nigdy wykrywania prędkości w scrollowanej treści. Przepisane w `usePanelSwipeToClose.ts` na framer-motion `useDragControls`+`dragListener={false}`: `.venue-panel-handle` zawsze inicjuje drag (niezależnie od scrolla), treść inicjuje go tylko gdy `scrollTop<=0`. Usunięty całkowicie poprzedni mechanizm wykrywania prędkości. Przy okazji naprawiony bug typów: `dragTransition` wymaga `InertiaOptions` (`bounceStiffness`/`bounceDamping`), nie generycznego springa — ujawnił się dopiero teraz, bo wcześniej `dragTransition` nigdy nie było faktycznie spreadowane na `motion.aside` w konsumentach. Zweryfikowane: `tsc`/`test`/`lint`/`build` czyste. Pełny opis: `docs/DECISIONS.md`.

**Runda 5 (2026-07-21)**: właściciel projektu przetestował uchwyt+pull-from-top na telefonie — działa, ale animacja/obsługa swipe'ów przy chowaniu/rozwijaniu kart na mobile wciąż "momentami nie wystarczająco płynna i intuicyjna". Świadoma decyzja: **nie dopracowywać tego teraz** — zostaje bez zmian, temat zapisany jako osobny przyszły **Etap K** (`docs/ROADMAP.md`, szczegóły w `docs/BACKLOG.md`, uzasadnienie w `docs/DECISIONS.md`). Nie blokuje dalszej pracy.

## Etap F — porządki techniczne (2026-07-21, 3 z 4 zrobione)

- ✅ `requireDb()` skonsolidowany do `src/lib/firebase.ts`, 4 serwisy zaktualizowane.
- ✅ Spójna walidacja współrzędnych — `isValidCoordinates` we współdzielonym `src/utils/geo.ts`.
- ✅ Martwe pole `status` usunięte całkowicie (typy, walidacja, normalizacja, `localBackupService`).
- ⏸️ **Pozostaje, celowo nieruszane teraz**: redukcja duplikacji `VenuePanel`/`EventPanel` — oba komponenty dopiero co dostały nową logikę gestów w tej samej sesji, większy refaktor odłożony do osobnego zadania (patrz `docs/BACKLOG.md`).

Zweryfikowane po każdym kroku: `npx tsc --noEmit`, `npm run test` 85/85, `npm run lint`, `npm run build` — wszystko czyste. Pełny opis: `docs/DECISIONS.md`. `docs/RISKS.md`/`docs/ROADMAP.md`/`docs/ARCHITECTURE.md`/`docs/DATA_MODEL.md`/`docs/GLOSSARY.md` zaktualizowane.

**Następny krok**: zapytać właściciela projektu, czy commitować+wypchnąć Etap F na `main`, i czy chce teraz podjąć redukcję duplikacji VenuePanel/EventPanel czy zostawić w backlogu. QA na telefonie dla reszty `docs/QA_CHECKLIST.md` (Cloudinary admin cover image, GitHub account linking, taken-username message) wciąż czeka na właściciela projektu równolegle.
